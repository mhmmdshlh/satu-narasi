...
export const AdminDashboard = () => {
    ...
    return (
        <div>
            ...
            <p className="text-gray-500 mb-8">Selamat datang, {profile?.full_name || profile?.username}</p>
            ...
            <h2 className="text-xl font-black text-gray-900 mt-10 mb-4">Status Laporan Warga</h2>
            <div className="grid md:grid-cols-3 gap-6">
                <div className="bg-yellow-50 border-l-4 border-yellow-500 p-6 rounded-xl">
                    <p className="text-3xl font-black text-yellow-700">{stats.pendingReports}</p>
                    <p className="text-yellow-700 font-medium">Laporan Pending</p>
                </div>
                <div className="bg-green-50 border-l-4 border-green-500 p-6 rounded-xl">
                    <p className="text-3xl font-black text-green-700">{stats.approvedReports}</p>
                    <p className="text-green-700 font-medium">Laporan Disetujui</p>
                </div>
                <div className="bg-red-50 border-l-4 border-red-500 p-6 rounded-xl">
                    <p className="text-3xl font-black text-red-700">{stats.rejectedReports}</p>
                    <p className="text-red-700 font-medium">Laporan Ditolak</p>
                </div>
            </div>
        </div>
    );
...
const StatCard = ({ icon, title, description, variant }) => (
    <div className={`p-6 rounded-xl ${variant === 'red' ? 'bg-red-50 border-l-4 border-red-500' : 'bg-gray-50 border-l-4 border-gray-500'}`}>
